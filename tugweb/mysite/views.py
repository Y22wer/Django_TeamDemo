...
def yolo_trans(img):
    ...
    path_weightfile = "C:/aiot_project/tugweb/absolute/path/to/best.pt"
...
class show_class(View):
    def post(self,request):
        form = ImageUploadForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            uploaded_img_form=ImageModel.objects.filter().last()
            img_bytes=uploaded_img_form.image.read()
            yoly_Img=yolo_trans(img_bytes)
            inference_img=uploaded_img_form.image_out(yoly_Img)
            yoly_Img.render()
            for img in yoly_Img.imgs:
...
def show(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            results = model(img, size=640)
            inference_img=uploaded_img_qs.image_out(results)
